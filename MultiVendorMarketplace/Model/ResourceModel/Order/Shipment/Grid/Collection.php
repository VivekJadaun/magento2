<?php
namespace Vinsol\MultiVendorMarketplace\Model\ResourceModel\Order\Shipment\Grid;
...
class Collection extends \Magento\Sales\Model\ResourceModel\Order\Shipment\Grid\Collection
{
    ...
    const JOIN_TABLE = \Vinsol\MultiVendorMarketplace\Model\VendorShipment::VENDOR_SHIPMENT_TABLE;
    ...
    public function _initSelect()
    {
        ...
        if ($this->appState->getAreaCode() == 'adminhtml') {
            if (($currentUser->getRole()->getRoleName() == self::VENDOR_ROLE_GROUP) && $currentUser->getIsActive()) {
                ...
                $this->getSelect()->join(self::JOIN_TABLE, 'entity_id = shipment_id')->where("user_id = ?", $currentUserId);
            }
        }
